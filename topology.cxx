...
topology::topology::topology(uint64_t inputSize, uint64_t nNeurons) {
    uint64_t nSynapses = inputSize;

    fLayers.push_back(std::make_pair<neuralLayer, synapseLayer>(neuralLayer(nNeurons, fMaxDepth++),
                                                                synapseLayer(nSynapses, fMaxDepth++)));
}
...
void topology::topology::fwdPropagate()
{
    for (auto &itLayers : fLayers) {
        ...
        for (auto &itSynapse : itLayers.second.fSynapses) {
            fLayers[itSynapse.fReceiverID[0]].first[itSynapse.fReceiverID[1]].setInput(itSynapse.fSenderID[1],
                                                                                       fLayers[itSynapse.fSenderID[0]].first[itSynapse.fSenderID[1]].getOutput());
        }
    }
}
...
void topology::topology::bckPropagate()
{
    for (auto &itLayers : fLayers) {
        for (auto &itSynapse : itLayers.second.fSynapses) {
            fLayers[itSynapse.fReceiverID[0]].first[itSynapse.fReceiverID[1]].setInput(itSynapse.fSenderID[1],
                                                                                       fLayers[itSynapse.fSenderID[0]].first[itSynapse.fSenderID[1]].getOutput());
            fLayers[itSynapse.fReceiverID[0]].first[itSynapse.fReceiverID[1]].setCorrection(itSynapse.fSenderID[0],
                                                                                            fLayers[itSynapse.fSenderID[0]].first[itSynapse.fSenderID[1]].getCorrection(
                                                                                                    itSynapse.fSenderID[1]));
        }
...
void topology::topology::train() {

    fwdPropagate();
    bckPropagate();
}

topology::topology &topology::topology::addLayer(uint64_t nNeurons) {
    uint64_t nSynapses = (*(fLayers.end())).first.fNeurons.size();
    fLayers.push_back(std::make_pair<neuralLayer, synapseLayer>(neuralLayer(nNeurons, fMaxDepth++),
                                                                synapseLayer(nSynapses, fMaxDepth++)));
    return *this;
}
